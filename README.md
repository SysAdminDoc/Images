...
- **Image-first viewer layout** — a quiet file-context bar and framed stage keep the active image dominant, while a compact navigation rail, progressive Details inspector, image-only filmstrip, grouped transport dock, and hover-reveal arrows keep controls clear without crowding the workspace.
...
- **Local semantic search** — open Semantic search from the context menu or Automation card to explicitly index selected folders into app-local `semantic-index.db`, filter by folder, reveal/open results, cancel indexing, and delete derived search data. It uses approved, user-imported CLIP ViT-B/32 ONNX models when their pinned hashes validate and otherwise falls back visibly to deterministic offline metadata embeddings.
- **Local face workflow CLI** — after manually importing the pinned OpenCV YuNet and SFace models, `Images.exe --face-detect <imagePath>` emits reviewable bounds, landmarks, confidence, and normalized region JSON; `--face-xmp <imagePath>` emits a literal MWG-rs XMP draft with unassigned regions; and `--face-cluster <imagePath> <imagePath> [...]` quality-gates 40 px faces, aligns five landmarks, creates private normalized SFace embeddings, and reports local cosine clusters without exposing the vectors. All three run through the shared Windows ML/DirectML/CPU router without opening a window or modifying images/sidecars.
