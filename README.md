...
- **Archive book previews** for ZIP and CBZ. Images opens supported image entries as read-only pages, promotes explicit cover/front images, ignores unsafe or nested archive entries, keeps archive navigation inside the existing page controls with a scrubber for quick jumps, remembers the last read page locally, and lists recent books with progress in the side panel.
