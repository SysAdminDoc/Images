...
- **Archive book previews** for ZIP and CBZ. Images opens supported image entries as read-only pages, promotes explicit cover/front images, ignores unsafe or nested archive entries, keeps archive navigation inside page controls plus a book-specific side-panel control card, remembers the last read page locally, and lists recent books with progress in the side panel.
