...
- **Rebuildable watched-folder catalog** — Images maintains an app-local SQLite catalog cache for library-scale workflows. Registered roots persist across launches, scan on normal startup, and refresh through debounced recursive file-system events; offline or ejected roots keep their cached rows until they reconnect or are explicitly removed. The catalog records source path, SHA-256 fingerprint, dimensions, file dates, codec metadata, XMP sidecar rating/tags, GPS, capture time, and camera/lens facts; sidecars and source files remain authoritative, so deleting `catalog.db` is a valid recovery step. `--catalog-search "<terms>"`, `--catalog-near <lat> <lon> <radiusKm>`, `--catalog-root-add <folder>`, `--catalog-root-remove <folder>`, `--catalog-root-list`, and `--catalog-rescan` provide window-free automation. Catalog migrations run forward only with integrity checks, WAL checkpointing, versioned backups, and a schema canary before the cache is reused.
