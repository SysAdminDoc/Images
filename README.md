...
- **Local face-detection CLI** — after manually importing the pinned OpenCV YuNet model, `Images.exe --face-detect <imagePath>` runs through the shared Windows ML/DirectML/CPU router without opening a window and emits reviewable JSON containing pixel bounds, five landmarks, confidence, and normalized MWG-compatible region coordinates. It never modifies the image or sidecar.
