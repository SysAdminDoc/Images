...
- **Destructive crop apply** — normal images open directly in freehand crop mode, so you can drag a crop rectangle immediately. Press `C` to pause/resume crop mode, choose free/square/3:2/4:3/16:9/custom aspect ratios, then press Enter, the side-panel Apply button, or the Apply button attached to the crop box to overwrite the file with the cropped image and notify Explorer so thumbnails refresh.
