...
- **Image-first viewer layout** — a quiet file-context bar and large stage keep the active image dominant, while a compact navigation rail, 316 px Details inspector, image-only filmstrip, and flat transport groups keep controls readable without surrounding every action or fact with a box. The shared desktop visual system uses 14 px body type, sentence-case section labels, underline tabs, restrained dividers, and outlines only where focus, selection, input, or structure needs them.
