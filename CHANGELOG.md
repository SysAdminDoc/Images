...
- **Deep-zoom tile engine foundation** — a tile pyramid generator and disk cache now break huge images (>256 MB or >50 megapixels) into DZI-format WebP tile levels with configurable tile size and overlap. The tile service computes source keys from path, size, and modification time, generates multi-level pyramids through Magick.NET, caches tiles at `%LOCALAPPDATA%\Images\tiles\`, and exposes tile query, load, and cache-clear APIs. The WPF tile-rendering viewport is future work.
