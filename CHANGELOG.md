...
- Added catalog-backed near-duplicate stacks (catalog schema v4). Incremental scans compute one shared, endian-stable 64-bit average hash per asset and backfill migrated rows once; the grouping engine combines bounded hash distance with capture/file time, aspect ratio, and optional GPS distance, excludes exact SHA-256 copies, prevents transitive location/hash bridges, and chooses a deterministic rated/high-resolution cover. `--catalog-stacks [<maxHashDistance> <maxSeconds> <maxMeters>]` emits one JSON object per stack without opening a window, while Duplicate Cleanup now reuses the same hash implementation.
- Completed Category Sets in the Tag Graph editor. Named layouts now persist through the existing bounded keyword-set store, can replace their saved tags, and swap the editor between job-specific tag panels without writing image metadata; sidecar mutation remains behind the explicit Export action. The compact selector supports save/use/delete, localized and pseudo-localized copy, accessible names, and caps each set at 256 tags and the store at 100 sets.
- Added persistent watched library roots. Semantic Search and the new window-free `--catalog-root-add`, `--catalog-root-remove`, `--catalog-root-list`, and `--catalog-rescan` commands share the catalog registry; normal app startup scans registered roots, recursive file-system events debounce into delta rebuilds, and a periodic registry refresh discovers roots changed by another process. Multi-root rebuilds retain cached assets and last-scan metadata while a drive is offline, clearly report the offline state, and resume scanning after reconnection instead of treating an eject as deletion.
