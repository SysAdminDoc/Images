...
- Began the live Skia canvas migration: ordinary static `BitmapSource` images now paint through a software SkiaSharp WPF surface with premultiplied-alpha, centered-uniform fit, and nearest-neighbor sampling parity. The existing WPF image remains an explicit fallback and continues to own animation, tile-pyramid, loupe, checkerboard, and overlay compatibility while later slices move those contracts deliberately.
