...
- **Local model/runtime manager** — the app now has a Model manager surface and service for approved local ONNX model definitions, app-local `models\inpaint` storage, manual import/delete/reveal actions, pinned SHA-256 validation for the OpenCV and Carve LaMa candidates, Windows ML versus ONNX Runtime DirectML readiness copy, and diagnostics provenance rows without automatic model downloads.
