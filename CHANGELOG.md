...
- Reworked the visual system against the image-generated `premium-viewer-v0.3.1-concept.png` target. All 23 WPF windows now inherit readable 14 px body type, sentence-case section labels, compact 52 px tool headers, denser dividers, flat secondary actions, underline tabs, borderless status labels, and unboxed content groups; outlines remain for input fields, focus, selection, modal surfaces, and major workspace boundaries. The main viewer now uses a slimmer 316 px Details inspector with aligned metadata rows instead of fact tiles, shorter workflow copy, a flatter command bar/transport grouping, and controls placed higher through reduced shell padding. Settings, About, confirmation, and crash surfaces received dedicated density passes, while visual-system contracts protect the shared hierarchy and mockup structure.
