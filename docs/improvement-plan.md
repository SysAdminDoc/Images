...
- 2026-05-17: Completed `V7-06` and `V7-07` by approving libjpeg-turbo 3.1.4.1 `jpegtran.exe` plus `jpeg62.dll` for release staging, tracking license/provenance files, adding a SHA-verified staging script, fixing redirected CLI diagnostics output, and wiring portable plus installed release diagnostics smoke into the workflow.
...
- 2026-05-17: Historical note: `V7-13` added XMP-backed rating and culling controls, gallery smart-filter refresh after sidecar writes, and regression coverage. That workflow was retired in v0.2.20 during viewer debloat and is not active product guidance.
...
- 2026-05-14: Advanced roadmap `V30-02` by adding explicit right-angle rotation writeback, exact aligned `jpegtran -rotate` shell-out with fake-process coverage, output validation, atomic replacement, rollback cleanup, raster fallback, viewer refresh, and Explorer thumbnail notification. Approved artifact staging and trim confirmation remained after that slice.
- 2026-05-14: Advanced roadmap `E12` / `V30-02` by adding interactive trim confirmation for JPEG crop and rotation overwrite, including confirmed trimmed `jpegtran` execution, exact raster re-encode fallback, and trim-path regression coverage. Approved artifact staging remains.
- 2026-05-14: Completed roadmap `V30-10` by adding a modeless perspective-correction workbench with four draggable source-corner handles, keystone nudges, XMP edit-stack persistence, Magick.NET perspective rendering, shortcut/menu routing, and export coverage.
- 2026-05-14: Completed roadmap `V30-11` by adding one-click Auto Enhance as a non-destructive edit-stack operation with automatic gamma, white balance, sigmoidal contrast, mild sharpening, shortcut/menu routing, and export coverage.
...
- 2026-05-14: Completed roadmap `V30-08` by adding a modeless annotations workbench with arrows, boxes, circles, text labels, numbered callouts, freehand pen strokes, blur/pixelate redaction, Magick.NET render support, XMP edit-stack persistence, and focused annotation/export coverage.
...
- 2026-05-05: Completed `IP-12A` and closed `IP-12` by adding generated decode/export corpus coverage for PNG, JPEG, WebP, multi-page TIFF, animated GIF, SVG vector decode, and APNG decode/fallback behavior without binary fixtures.
