...
dotnet publish src/Images -c Release -r win-x64 --no-self-contained -o publish
```

If `-Source` is omitted, the script checks `IMAGES_GHOSTSCRIPT_DIR` and standard `%ProgramFiles%\gs\gs*` installs.

The project copies `src/Images/Codecs/Ghostscript/**` into build and publish output. The installer packages the published output, so app-local Ghostscript files are included automatically once present during publish.
