...
| OpenCV YuNet and SFace ONNX | Accepted, opt-in models not bundled | Official OpenCV Zoo YuNet (MIT) and SFace (Apache-2.0) artifacts are pinned by repository revision, byte length, and SHA-256. Images only accepts manual imports and routes inference through the shared Windows ML / DirectML / CPU runtime. Embedding vectors stay private and are discarded before workbench state is built; regions reach XMP only after every detection is explicitly accepted/rejected and every accepted region is named. |
| Face-aware culling heuristics | Accepted, review-only local metrics | Face crops are normalized to 128x128 before Laplacian-variance sharpness measurement. Possible-closed-eyes uses only the horizontal-to-vertical texture ratio in two bounded patches centered on YuNet eye landmarks, and is withheld when the face is already locally blurred. Thresholds are visible constants with synthetic directional/blur tests. These signals never write Pick/Reject state or metadata and are not presented as biometric or medical conclusions. |
| OpenCV YOLOX-S ONNX | Accepted, opt-in model not bundled | The official OpenCV Zoo Apache-2.0 COCO detector is pinned by repository revision, byte length, and SHA-256. Object labels are local review suggestions; Images does not automatically modify source metadata. |
