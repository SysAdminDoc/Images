...
| jpegtran.exe | Conditionally accepted resolver/provenance and exact-crop shell-out | Lossless JPEG crop/rotation planning, runtime diagnostics, and exact MCU-aligned crop writeback are documented in `docs/lossless-jpeg-transform-policy.md`; bundling still requires staging the exact libjpeg-turbo Windows artifact, carrying license files, and recording SHA-256 provenance. Trim-confirmation UI, rotation writeback, and staged-runtime smoke coverage remain. |
