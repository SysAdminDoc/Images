...
- **Latest minor** — receives bug fixes, security fixes, and feature work. The current `0.2.x` line is on this footing.
- **Previous minor** — receives security fixes only, until the next minor lands.
- **All older versions** — best-effort; no fixes back-ported, no compatibility guarantees.

There is no LTS line yet; one will be considered when Images reaches `1.0`.
