...
- RAR/CBR and 7z/CB7 use `SharpCompress` 0.50.0 from NuGet.
...
- 2026-05-17 dependency refresh: 0.47.4 was flagged by GHSA-6c8g-7p36-r338 / CVE-2026-44788 for the `WriteToDirectory()` extraction helper. Images does not call that API, but the package was upgraded to 0.49.1 so the solution vulnerability gate is clean again.
- 2026-07-16 dependency refresh: upgraded to 0.50.0 for the LZMA/RAR decode allocation reductions and Zip64 non-seekable-stream / entry-metadata-corruption fixes. The 0.50.0 breaking changes (Tar streams no longer auto-decompress; Detection API reshaped) do not affect Images, which reads only ZIP/CBZ/RAR/CBR/7z/CB7 through `ArchiveFactory.OpenArchive` and `IArchive.Entries` — no tar or detection-API usage. The generated CBZ/CBR/7z/CB7 regression suite passes unchanged.
