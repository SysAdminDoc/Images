...
- `Services/ExportCapabilityWarningService.cs` - shared export/batch warnings for target-format losses.
- `Services/ImageColorAnalysisService.cs` - read-only ICC/profile and histogram/channel summaries for the side panel.
...
- `Services/ModelManagerService.cs` - approved local model registry, app-local grouped storage, SHA-256 import validation, delete/reveal support, and runtime readiness status for future local inference tools. Current pinned candidates include OpenCV/Carve LaMa for inpaint and Qdrant CLIP ViT-B/32 text/vision ONNX files for semantic search.
...
- Structured runtime/dependency provenance rows in About, `--system-info`, and `--codec-report`, covering NuGet packages, optional runtimes, OS OCR, and local model/runtime readiness with source, version, path, SHA-256 where available, advisory status, and action copy.
...
- Export preview workbench with original versus in-memory encoded preview, JPEG/PNG/WebP/AVIF/JXL presets, quality and max-dimension controls, estimated output size and byte delta, resize-aware save output, and shared target-format warnings for alpha flattening, animation frame loss, page/layer flattening, EXIF/IPTC/XMP metadata, ICC profile risk, and lossy quality settings. Batch preview and dry-run paths use the same warning service.
- Side-panel color analysis with embedded ICC/profile status, decoded color space, sampled luma and RGB channel stats, shadow/midtone/highlight histogram percentages, alpha transparency stats, and explicit unmanaged-color warnings without changing pixels.
...
- Local Model manager for approved ONNX model files: manual import/delete/reveal workflow, app-local grouped storage, pinned SHA-256 validation for OpenCV/Carve LaMa and Qdrant CLIP ViT-B/32 candidates, runtime status copy for Windows ML versus ONNX Runtime DirectML, and no automatic downloads.
...
- Semantic search has a usable local index/search foundation plus Qdrant CLIP text/vision ONNX hash pins, but V7-31 remains open until tokenizer/preprocessor import policy, an approved ONNX image/text embedding provider, runtime package selection, and model execution validation land. Background removal, upscaling, and inpainting still need feature-specific inference UI/provenance before shipping.
...
1. Continue `V7-31` by defining tokenizer/preprocessor file handling for the pinned Qdrant CLIP models, then replace or augment the deterministic metadata provider with an approved ONNX image/text embedding provider behind the existing `ISemanticEmbeddingProvider` seam.
