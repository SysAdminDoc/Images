...
- Local Model manager for approved model files: manual import/delete/reveal workflow, app-local grouped storage, pinned SHA-256 validation for OpenCV/Carve LaMa and Qdrant CLIP ViT-B/32 candidates, runtime status copy for Windows ML versus ONNX Runtime DirectML, and no automatic downloads.
...
- Semantic search has a usable local index/search foundation plus Qdrant CLIP text/vision ONNX, tokenizer, and preprocessor hash pins, but V7-31 remains open until an approved ONNX image/text embedding provider, runtime package selection, and model execution validation land. Background removal, upscaling, and inpainting still need feature-specific inference UI/provenance before shipping.
...
1. Continue `V7-31` by replacing or augmenting the deterministic metadata provider with an approved ONNX image/text embedding provider behind the existing `ISemanticEmbeddingProvider` seam, using the pinned Qdrant CLIP text/vision/tokenizer/preprocessor files.
