...
- Semantic search foundation with explicit selected-folder indexing into app-local `semantic-index.db`, an embedding-provider seam, deterministic offline metadata embeddings, exact cosine search, folder filter, result open/reveal, cancellation, delete-index controls, and Qdrant CLIP ViT-B/32 text/vision ONNX hash pins in Model manager. This is not yet an approved ONNX CLIP/SigLIP runtime because tokenizer/preprocessor handling and model execution validation remain open.
