...
- `src/Images/ModelManagerWindow.xaml`, `src/Images/ModelManagerWindow.xaml.cs`, and `src/Images/Services/ModelManagerService.cs` - generalize import wording from ONNX-only to approved model files and add pinned Qdrant CLIP ViT-B/32 text, vision, tokenizer, and preprocessor candidates for the next semantic-search runtime slice.
